...
class Sales extends Secure_area
{
	...
	function complete()
	{
		...
		if($customer_id!=-1)
		{
			...
			$cust_info=$this->Customer->get_info($customer_id);
			$data['nit']=$cust_info->phone_number;
		}

		//SAVE sale to database
		$data['sale_id']='Factura No. '.$this->Sale->save($data['cart'], $customer_id,$employee_id,$comment,$data['payments'], $this->sale_lib->get_suspended_sale_id(), 0);
	...
	function email_receipt($sale_id)
	{
		...
		$data['sale_id']='Factura No. '.$sale_id;
	...
	function suspend()
	{
		...
		//SAVE sale to database
		$data['sale_id']='Factura No. '.$this->Sale->save($data['cart'], $customer_id,$employee_id,$comment,$data['payments'], $sale_id, 1);
		if ($data['sale_id'] == 'POS -1')
		{
			$data['error_message'] = lang('sales_transaction_failed');
		}
